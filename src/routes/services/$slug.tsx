...
export const Route = createFileRoute("/services/$slug")({
  loader: ({ params }) => {
    ...
    const { icon: _icon, ...plain } = service;
  ...
  head: ({ loaderData }) => {
    ...
    const description =
      `${s.subtitle} ${SITE.name}, ψυχολόγος Γαλάτσι. Επικοινωνήστε για το πρώτο σας ραντεβού.`.slice(
        0,
        160,
      );
      ...
      <section className="border-b border-border bg-secondary/40 py-12 md:py-20">
        <div className="mx-auto max-w-4xl px-4 md:px-8">
          ...
          <div className="mt-6 flex items-start gap-5">
            ...
            <div>
              <h1 className="font-serif text-3xl leading-tight text-ink md:text-5xl">{s.title}</h1>
      ...
      <section className="bg-background py-16 md:py-24">
        <div className="mx-auto grid max-w-4xl gap-12 px-4 md:grid-cols-[1.5fr_1fr] md:px-8">
          <div className="space-y-12">
            ...
            {s.whenItHelps && <PillList title="Πότε μπορεί να βοηθήσει" items={s.whenItHelps} />}

            {s.howWeWork && <NumberedList title="Πώς δουλεύουμε" items={s.howWeWork} />}

            {s.approaches && (
              <PillList title="Θεραπευτικές προσεγγίσεις & εργαλεία" items={s.approaches} />
            )}

            {s.ageGroups && <PillList title="Ανά ηλικιακή ομάδα" items={s.ageGroups} />}

            {s.whatYouGain && <CheckList title="Τι θα κερδίσετε" items={s.whatYouGain} />}
          ...
          <aside className="h-fit rounded-2xl border border-border bg-card p-6 shadow-sm md:sticky md:top-28">
            ...
            <h3 className="mt-2 font-serif text-xl text-ink">Επικοινωνήστε μαζί μου</h3>
            <Link
              to="/contact"
